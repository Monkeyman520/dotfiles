...
bash -c 'stow -D -d "$HOME/dotfiles" -t "$HOME/" .'
...
# 包列表
packages=("zsh" "git" "tmux" "vfox" "fd" "ripgrep" "fzf" "neovim" "stow" "starship")
...
bash -c 'cd "$HOME/dotfiles" && git restore . && stow --adopt -d "$HOME/dotfiles" -t "$HOME/" .'

/bin/zsh -c "nvim --headless -c 'quitall'"
